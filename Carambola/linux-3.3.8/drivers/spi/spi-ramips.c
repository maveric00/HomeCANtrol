...
#define GPIO_SECOND_CS 14

#define DRIVER_NAME			"ramips-spi"
#define RALINK_NUM_CHIPSELECTS		2 /* two slaves are supported*/
...
static void ramips_spi_set_cs(struct ramips_spi *rs, int cs, int enable)
{
  spi_debug("%s: set_cs : %d\n", __func__,enable);
  if (cs==0) {
    if (!enable) {
      spi_debug("%s: CLR SPIENA\n", __func__);    
      ramips_spi_setbits(rs, RAMIPS_SPI_CTL, SPICTL_SPIENA);
    } else {
      spi_debug("%s: SET SPIENA\n", __func__);    
      ramips_spi_clrbits(rs, RAMIPS_SPI_CTL, SPICTL_SPIENA);
    } ;
  } else {
    if (enable) {
      spi_debug("%s: SET PIO14\n", __func__);    
      gpio_set_value(GPIO_SECOND_CS,0) ;
    } else {
      spi_debug("%s: CLR PIO14\n", __func__);    
      gpio_set_value(GPIO_SECOND_CS,1) ;
    } ;
  } ;
}
	...
	if (tx && rx) {
	  ...
	  for (count = 0; count < xfer->len; count++) {
	    ...
	    if ((count == 0) || (mcp2515_command & 0x40) || (mcp2515_command == 0x05) || (mcp2515_command == 0x02)) {
	      spi_debug("%s: r/w w [%d] 0x%02x\n", __func__,count, tx[count]);
...
static void ramips_spi_work(struct work_struct *work)
{
	...
	while (!list_empty(&rs->msg_queue)) {
		...
		list_for_each_entry(t, &m->transfers, transfer_list) {
			...
			if (!cs_active) {
			  ramips_spi_set_cs(rs,spi->chip_select, 1);
			  cs_active = 1;
			}
			...
			if (t->cs_change) {
			  ramips_spi_set_cs(rs,spi->chip_select, 0);
				cs_active = 0;
			}
		}
...
static int ramips_spi_setup(struct spi_device *spi)
{
	...
	if (spi->chip_select==1) {
	  gpio_request_one(GPIO_SECOND_CS,GPIOF_DIR_OUT|GPIOF_INIT_HIGH,"Second CS") ;
	} ;
	/*
...
static void __init ramips_spi_reset(struct ramips_spi *rs)
{
  ...
  //			 SPICFG_SPICLK_DIV16 | SPICFG_SPICLKPOL);
	ramips_spi_write(rs, RAMIPS_SPI_CFG, SPICFG_RXCLKEDGE_FALLING |
			 SPICFG_MSBFIRST | SPICFG_TXCLKEDGE_FALLING|
			 SPICFG_SPICLK_DIV16);
