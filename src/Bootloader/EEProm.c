...
int main (int argc, char** argv)
{
  ...
  Config.Magic[1] = 0xCA ;
  Config.PAD = 0xFF ;
  for (i=0;i<502;i++) Config.Data.PAD[i] = 0xFF ;

  i = -1 ;
  while (i<0) {
    printf ("Knotentyp (0: LED, 1: Relais, 2: Sensor): ") ;
    scanf ("%d",&i) ;
    if ((i!=0)&&(i!=1)&&(i!=2)) { 
      printf ("Unbekannter Knotentyp\n") ;
      i = -1 ;
    } ;
  }; 
  Config.BoardType = (unsigned char)(i*16) ;

  i = -1 ;
  while (i<0) {
    printf ("Knoten-Addresse (0-1023): ") ;
    scanf ("%d",&i) ;
    if ((i<0)||(i>1023)) { 
      printf ("Addresse nicht moeglich\n") ;
      i = -1 ;
    } ;
  }; 
  Config.BoardAdd[0] = (unsigned char)(i&0xff) ;
  Config.BoardAdd[1] = (unsigned char)(i>>8) ;

  i = -1 ;
  while (i<0) {
    printf ("Knoten-Linie (1-4): ") ;
    scanf ("%d",&i) ;
    if ((i<1)||(i>4)) { 
      printf ("Linie nicht moeglich\n") ;
      i = -1 ;
    } ;
  }; 
  Config.BoardLine = (unsigned char)i ;
  ...
  //Erzeugen der EEprom.eep-Datei
  
  Buffer =  (unsigned char*)& Config ;

  for (i=0;i<16;i++) { /* 16 Records schreiben */
    New_IHexRecord(IHEX_TYPE_00,i*32,Buffer,32,&IR) ;
    Write_IHexRecord(&IR,OutFile); 
    Buffer+=32 ;
  } ;
  New_IHexRecord (IHEX_TYPE_01,0,Buffer,0,&IR) ;
  Write_IHexRecord(&IR,OutFile); 
  fclose (OutFile) ;
  

  if (Config.BoardType==32) {
    sprintf (Command,"avrdude -v -p t84 -c avrisp2 -P usb -U lfuse:w:0xe2:m -U hfuse:w:0xd2:m -U efuse:w:0xfe:m") ;
  } else {
    sprintf (Command,"avrdude -v -p at90pwm3b -c avrisp2 -P usb -U lfuse:w:0xC3:m -U hfuse:w:0xd2:m -U efuse:w:0xf8:m") ;
  } ;

  printf (Command) ;

  if (Config.BoardType==32) {
    sprintf (Command,"avrdude -p t84 -P usb -c avrisp2 -U flash:w:bootloader_T.hex:i") ;
  } else {
    sprintf (Command,"avrdude -p at90prm3b -P usb -c avrisp2 -U flash:w:bootloader_M.hex:i") ;
  } ;

  printf (Command) ;

  sprintf (Command,"avrdude -p %s -P usb -c avrisp2 -U eeprom:w:eeprom_b.eep:i",Config.BoardType==32?"t84":"at90pwm3b") ;
